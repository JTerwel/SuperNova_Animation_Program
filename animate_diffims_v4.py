...
#Version 4 also shows the location of the center of the psf that was used to get the lc data point
...
def main(): #main function
	...
	t = time.Time([58360, 58361, 58390, 58392, 58480, 58740], format='mjd').jd
	...
	save_name = 'ZTF18abmxfrc_late_zquery1_14_1.mp4' #Name under which the animation is saved
...
def animate_observations(obj_loc, obj_name, im_locs, lc_dat, t_frame, min_visible_region, cutout_dims, central_region):
	...
	lc_loc = ax_im.scatter([], [], color='r', marker='+')
	...
	movers = [im, line_hor, line_ver, wire, text, lc, lc_line, lc_loc]
...
def animate_step(im_dat, movers, ax_im, ax_wire, ax_ne, lc_dat, central_region, obj_name):
	#im_dat is the following collection: [cutout, imtype, date, obsfilter, im_center]
	#Get values needed to plot this frame
	if im_dat[1]=='ref':
		lc_points = [[],[]]
		lc_locs = [np.array([-999]), np.array([-999])] #There should not be anything here, but it is a bit of a makeshift solution so be careful
	else:
		lc_points, lc_locs = find_lc_point(im_dat[2], im_dat[3], lc_dat)
	data, theta, lc_locs = find_orientation(im_dat[0], im_dat[4], lc_locs) #theta to be used for arrows to N, E
...
def find_orientation(im, im_center, lc_locs): #Checks if im is mirrored, and finds direction of non-mirrored image east
	if lc_locs == []: #In case there was no lc point found
		lc_locs =  [np.array([-999]), np.array([-999])]
	wcs = im.wcs
	...
	diff_x = lc_locs[0]-im_center[0] #x offset from image center
	if (((to_east[0]>0) & (to_north[1]<0)) | ((to_east[0]<0) & (to_north[1]>0))):
		data = np.flipud(im.data)
		theta = np.arctan2(-to_east[1], to_east[0])
		diff_y = im_center[1]-lc_locs[1] #y offset from image center after flip
	else: #image is not mirrored
		data = im.data
		theta = np.arctan2(to_east[1], to_east[0])
		diff_y = lc_locs[1]-im_center[1] # y offset from image center
	...
	rots = int(theta/90) #nr of clockwise rotations
	data = np.rot90(data, rots, (1,0)) #rotated data
	#add rotations and move origin from center of the image to the actual origin
	if rots%4==1: #1 time clockwise
		locs = [diff_y+np.shape(data)[0]/2, -diff_x+np.shape(data)[1]/2]
	elif rots%4==2: #im was upside down
		locs = [-diff_x+np.shape(data)[0]/2, -diff_y+np.shape(data)[1]/2]
...
def find_lc_point(date, im_filter, lc_dat): #Finds points in light curve that belong to the image
	...
	if len(good_dates[good_dates.obs_filter==im_filter])==0: #no matches
		return [[],[]]
	vals = []
	pix_x = [] #pixel location of psf center
	pix_y = []
	for i in range(len(good_dates[good_dates.obs_filter==im_filter])):
		...
		pix_x.append(good_dates[good_dates.obs_filter==im_filter].target_x.iloc[i])
		pix_y.append(good_dates[good_dates.obs_filter==im_filter].target_y.iloc[i])
	return vals, [np.array(pix_x), np.array(pix_y)]
...
def get_lc(lc_loc, lc_dates):
	lc = pd.read_csv(lc_loc, header=0, usecols=['obsmjd', 'filter', 'upper_limit', 'mag', 'mag_err', 'target_x', 'target_y'])
