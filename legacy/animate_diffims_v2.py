...
#If central pixel (where [ra,dec] is focused on) is outside image, it is skipped in the animation
...
def main(): #main function
	...
	#RELOCATE save_name!
	save_name = 'ZTF18abmxfrc_late_full.mp4' #Name under which the animation is saved
	...
	t = time.Time([58355, 58365, 58480, 58730], format='mjd').jd
...
def animate_step(i, cutouts, im_dat, lc_dat, movers, ax_wire):
	...
	movers[1].axes.set_ylim(np.nanmin(points_hor)-1, np.nanmax(points_hor)+1)
	movers[2].axes.set_xlim(np.nanmin(points_ver)-1, np.nanmax(points_ver)+1)
...
def gen_cutouts(cutout_dims, radec, refquery, difquery, obj_name):
	...
	for i in range(len(data_locs)):
		...
		try: #See if the target coord is in the image, if not go to next image
			...
			continue
		cutouts.append(Cutout2D(im[-1].data, pix_xy, cutout_dims, mode='partial')) #Do the cutout, pad if needed
