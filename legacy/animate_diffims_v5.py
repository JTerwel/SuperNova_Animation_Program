#Author: Jacco Terwel
#Date: 17 - 05 - 2021
#This program retrieves reference images and difference imagess of requsted
#epochs from ZTF. It then cuts out the requested region and puts it in an
#animation cycling through the images chronologically.
#It is made to easily add / remove subplots showing different things during
#the animation, and allows things like the size of the cutout image and time
#each frame is shown to be changed at will.
...
def main():
	...
	ra = 261.4131984
	dec = 59.4467859
	t = time.Time([58263, 58273, 58700, 58730], format='mjd').jd
	...
	lc_loc = '/Users/terwelj/Documents/ZTFData/marshal/'\
		'SN_Ia/ZTF18aasdted_SNT_1e-08.csv'
	lc_dates = [58200, 58900]
	#search size = 
	#t_frame = 
	#cutout_dims = 
	#min_visible = 
	...
	subplot_selection = [
		...
		init_text,
		init_lc]
	save_name = 'ZTF18aasdted_peak_and_late.mp4'
...
class anim_target:
	...
	def get_lc(self):
		...
		lc = pd.read_csv(self.lc_loc, header=0,
			usecols=['obsmjd', 'filter', 'upper_limit', 'mag', 'mag_err',
					'target_x', 'target_y'])
...
def get_next_im(target):
	...
	for i in target.im_locs:
		...
		yield [immat, imtype, date, obsfilter, rots, flip, resid, im_center]
...
def update_im(im_dat, target, subplot, mover, extra_args):
	...
	Parameters:
		im_dat ([immat, imtype, date, obsfilter, rots, flip, resid, im_center]):
			Required image data.
		target (anim_target): Object to animate.
	...
	lc_locs = find_fp_loc(target.lc, im_dat[2], im_dat[3], im_dat[4], im_dat[5],
		im_dat[7], target.cutout_dims)
...
def update_compass(im_dat, target, subplot, mover, extra_args):
	...
	#Add the new arrows.
	subplot[0].arrow(0.5, 0.5, 0.3*np.cos(im_dat[6]), 0.3*np.sin(im_dat[6]),
		 color='k', length_includes_head=True, head_width=0.1, head_length=0.1)
	subplot[0].arrow(0.5, 0.5, -0.3*np.sin(im_dat[6]), 0.3*np.cos(im_dat[6]),
		color='k', length_includes_head=True, head_width=0.1,
		head_length=0.1)

	#Add the new markers.
	subplot[0].text(0.5+0.4*np.cos(im_dat[6]), 0.5+0.4*np.sin(im_dat[6]), 'E',
		color='k', va='center', ha='center')
	subplot[0].text(0.5-0.4*np.sin(im_dat[6]), 0.5+0.4*np.cos(im_dat[6]), 'N',
		color='k', va='center', ha='center')
...
def update_text(im_dat, target, subplot, mover, extra_args):
	...
	text = gen_text(extra_args[0], im_dat[1], im_dat[2], im_dat[3], peak, mean,
		stddev)
...
def update_lc(im_dat, target, subplot, mover, extra_args):
	...
	if im_dat[1] =='ref':
		...
		lc_points = find_fp_points(target.lc, im_dat[2], im_dat[3])
		mover[1].set_xdata([im_dat[2].mjd, im_dat[2].mjd])
...
def find_fp_loc(lc, date, im_filter, rots, flip, im_center, cutout_dims):
	'''
	Find the location of the associated forced photometry point(s).

	Parameters:
		lc (DataFrame): lightcurve with forced photometry points.
		date (Time): Date of image.
		im_filter (string): Filter in which the image was taken.
		rots (int): Nr of times the image was rotated clockwise.
		flip (bool): was the image flipped?
		im_center (2 floats): pixel coordinates of cutout center.
		cutout dims (tuple or int): Size of the cutout image.
	...
	if lc is None or date == ' ':
		return [[None, None]]

	#Find all lc points within 8.64 s of the image in the correct filter
	good_dates = lc[(np.abs(lc.obsmjd-date.mjd)<1e-4) &
		(lc.obs_filter==im_filter)]
	if len(good_dates)==0:	#no matches
		return [[None, None]]
	xvals = good_dates.target_x.to_numpy().transpose()	#Put in simple arrays
	yvals = good_dates.target_y.to_numpy().transpose()

	#Calculate x & y values with respect to cutout center while taking the flip
	#into account if needed.
	diff_x = xvals-im_center[0]
	if flip:
		diff_y = im_center[1] - yvals
	else:
		diff_y = yvals - im_center[1]

	#Add rotations and move origin to the actual origin.
	if rots%4==1: #1 time clockwise
		locs = [diff_y+cutout_dims[1]/2, -diff_x+cutout_dims[0]/2]
	elif rots%4==2: #im was upside down
		locs = [-diff_x+cutout_dims[1]/2, -diff_y+cutout_dims[0]/2]
	elif rots%4==3: #1 time anticlockwise
		locs = [-diff_y+cutout_dims[1]/2, diff_x+cutout_dims[0]/2]
	else: #rot%4==0 means do nothing
		locs = [diff_x+cutout_dims[1]/2, diff_y+cutout_dims[0]/2]
...
def find_fp_points(lc, date, im_filter):
	'''
	Find the forced photometry point(s) belonging to the image.

	Parameters:
		lc (DataFrame): Lightcurve with forced photometry points.
		date (Time): Date of the image.
		obs_filter (string): Filter in which the image was taken.
...
def gen_text(name, im_type, date, obs_filter, peak, mean, stddev):
	'''
	Generate the text for the next frame.

	Parameters:
		name (string): Object name.
		im_type (string): Image type. (ref / diff)
		date (Time): Date of the image.
		obs_filter (string): Filter in which the image is taken.
		peak (float): Maximum value in the cutout image.
		mean (float): Mean value in the cutout image.
		stddev (float): standard deviation in the cutout image.
	...
	if im_type=='ref':
		return text.format(name, im_type, obs_filter, ' ', ' ', peak, mean,
			stddev)
	else:
		#Calculate the dat in Gregorian format (easier to read than mjd).
		greg = date.isot
		date_greg = '{}-{}-{} {}:{}:{}'.format(greg[8:10], greg[5:7], greg[:4],
			greg[11:13], greg[14:16], greg[17:])
		return text.format(name, im_type, obs_filter, date_greg, date.mjd, peak,
			mean, stddev)
